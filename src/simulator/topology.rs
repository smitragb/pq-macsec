...
use crate::{
    ...
    log_frame,
    nodes::{NodeHandler, NodeId, SimpleNode},
    ...
    fn handle_pkt(&mut self, event: Event) -> Option<Event> {
        match event {
            Event::SendPkt {
                ...
                let peer = self.get_peer_from_id(from, port);
                let peer_id = peer.id;
                let peer_port = peer.port;
                let mut link = self.get_link_from_id(from, port);
            ...
            Event::RcvPkt {
                ...
                let node = self.get_node_from_id(at, port);
